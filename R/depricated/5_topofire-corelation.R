...
read_topofire = function(x){
  station_meta = read_csv("~/nrcs-scd-soil-moisture-eval-data/processed/station-meta-conus-w-data-final.csv", show_col_types = F)
  data = read_csv(x, show_col_types = F) |>
    mutate(network = station_meta$network) |>
    select(network, site_id, everything())
  
  return(data)
}

topofire_data = list.files("~/nrcs-scd-soil-moisture-eval-data/raw/topofire/raw", full.names = T)%>%
  purrr::map(read_topofire) %>%
  bind_rows() 
...
topofire_infill = list.files("~/nrcs-scd-soil-moisture-eval-data/raw/topofire_infill/raw", full.names = T) |>
  ...
  select(network, site_id, date = time, Topofire = value)

topofire_dates = c(
  colnames(topofire_data)[3:length(colnames(topofire_data))] %>%
  as.Date(),
  unique(topofire_infill$date)
)
...
topofire_data_long = topofire_data %>% 
  pivot_longer(cols = -c(site_id, network)) %>%
  dplyr::select(network, site_id, date = name, Topofire = value) %>%
  drop_na(Topofire) %>%
...
params = read_csv("~/nrcs-scd-soil-moisture-eval-data/processed/station-covariates.csv") |>
  dplyr::select(network, site_id, topofire_aws, awc_gNATSGO, porosity_gNATSGO, fc_gNATSGO, aws0_999) |>
  left_join(max_topofire)

topofire_data_final = topofire_data_long %>% 
  left_join(params, c('network','site_id'))  %>%
  ...
  dplyr::select(network, site_id, date, Topofire = Topofire_VWC_old)

rm(topofire_data_long); gc(); gc()

data_joined = staion_data %>% 
  left_join(., topofire_data_final, by = c('network','site_id', 'date'))
...
site_depth_corelations = data_joined %>%
  pivot_longer(cols = -c(network, site_id_long, site_id, date, Topofire), names_to = 'generalized_depth', values_to = 'soil_moisture') %>%
...
scatter_model_vs_obs(
  data = data_joined %>%
    pivot_longer(cols = -c(network, site_id, site_id_long, date, Topofire), names_to = 'generalized_depth', values_to = 'soil_moisture') %>%
    drop_na(Topofire, soil_moisture),
  sims = Topofire,
  obs = soil_moisture,
  network = network,
