...
all_results = bind_rows(soilwat2)
...
model_diff_plot = function(data, variable, states,
  ...
  t.test(data_diff$Topofire, data_diff$SOILWAT2, paired = TRUE)
  wilcox.test(data_diff$Topofire, data_diff$SOILWAT2, paired = TRUE)
...
model_diff_plot(
  ...
  clamp_to_percentile = TRUE,
  depth_avg_only = F,
  export_path = "~/nrcs-scd-soil-moisture-eval/figs/diff-topofire-vs-soilwat2-NSE.png"
)
...
histogram_plot = function(data, variable, network, 
  ...
  # Label map for dynamic variable names
  var_label <- rlang::as_name(variable)
  ...
  plot_title <- if (!is.null(model)) paste(model, x_label) else paste("Model", x_label)
  subtitle_text <- if (depth_avg_only) "Depth Averaged Only" else ""
  
  # Tick label formatter (clamp-aware)
  label_func <- function(x) {
    formatted <- format(round(x, 2), nsmall = 2)
    if (clamp && length(x) > 0) {
      formatted[which.min(x)] <- paste0("<", formatted[which.min(x)])
    }
    formatted
  }
  ...
  hist_plot = ggplot(data, aes(x = !!variable, color = !!network)) +
    ...
    ggtitle(plot_title, subtitle = subtitle_text) +
    labs(y = 'Station Density', x = x_label, fill = NULL, color = NULL)
