...
make_eval_table <- function(all_results, outfile_png,
  ...
  tab_header(
    ...
    subtitle = md("Comparison of SOILWAT2 and Topofire models across networks")
  ) %>%
    # -------------------------
  tab_spanner("SOILWAT2", columns = c(SOILWAT2_r, SOILWAT2_KGE, SOILWAT2_Bias, SOILWAT2_RMSE)) %>%
    tab_spanner("Topofire", columns = c(Topofire_r, Topofire_KGE, Topofire_Bias, Topofire_RMSE)) %>%
    cols_label(
      SOILWAT2_r = html("<b>r</b>"), SOILWAT2_KGE = html("<b>KGE</b>"),
      SOILWAT2_Bias = html("<b>Bias</b>"), SOILWAT2_RMSE = html("<b>RMSE</b>"),
      Topofire_r = html("<b>r</b>"), Topofire_KGE = html("<b>KGE</b>"),
      Topofire_Bias = html("<b>Bias</b>"), Topofire_RMSE = html("<b>RMSE</b>")
    ) %>%
    tab_options(
...
make_eval_table_all_depths <- function(
  ...
  gt_tbl <-
    ...
    gt::tab_header(
      ...
      subtitle = gt::md("Comparison of SOILWAT2 and Topofire across networks and soil depths")
    ) %>%
    gt::tab_spanner("SOILWAT2",
                    columns = c(SOILWAT2_r, SOILWAT2_KGE, SOILWAT2_Bias, SOILWAT2_RMSE)) %>%
    gt::tab_spanner("Topofire",
                    columns = c(Topofire_r, Topofire_KGE, Topofire_Bias, Topofire_RMSE)) %>%
    gt::cols_label(
      SOILWAT2_r    = gt::html("<b>r</b>"),
      SOILWAT2_KGE  = gt::html("<b>KGE</b>"),
      SOILWAT2_Bias = gt::html("<b>Bias</b>"),
      SOILWAT2_RMSE = gt::html("<b>RMSE</b>"),
      Topofire_r    = gt::html("<b>r</b>"),
      Topofire_KGE  = gt::html("<b>KGE</b>"),
      Topofire_Bias = gt::html("<b>Bias</b>"),
      Topofire_RMSE = gt::html("<b>RMSE</b>")
    ) %>%
    gt::tab_options(
